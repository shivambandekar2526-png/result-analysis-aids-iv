...
def parse_mark_token(t, max_val):
    ...
    t = str(t).strip()
    if any(w in t.upper() for w in ["ABS", "AA", "AB", "NULL"]): return 0
...
def extract_student_deep(page_idx, sb, boxes):
    s_y = sb["ymin"]
    ...
    for subj in SUBJECTS:
        ...
        # 4. Slot 4: I1 (dy 79..104)
        if stype == "THEORY":
            for b in c_boxes:
                if 79 <= (b["ymin"] - s_y) <= 104:
                    v = parse_mark_token(b["text"], subj["max_int"])
                    if v is not None: int_m = v
                    
        # 5. Slot 5: TOT & Grade (dy 104..160)
        for b in c_boxes:
            if 104 <= (b["ymin"] - s_y) <= 160:
                t = b["text"]
...
for p in range(2, 47):
    ...
    for sb in seat_boxes:
        rec = extract_student_deep(p, sb, boxes)
        records.append(rec)
