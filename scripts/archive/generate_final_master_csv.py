...
def build_student_record(page_idx, sb, boxes):
    s_y = sb["ymin"]
    ...
    # 1. Extract raw component marks for each subject
    comp_data = {}
    ...
    # 2. Extract Bottom Row Chain (TOT, Grade, GP, Credits, GC)
    ...
    for i, s in enumerate(SUBJECTS):
        ...
        if stype == "THEORY":
            if cd["ext"] is not None and cd["int"] is not None:
                ...
                if subj_totals[i] is None or abs(subj_totals[i] - comp_tot) > 20:
                    subj_totals[i] = comp_tot
                elif subj_totals[i] != comp_tot:
                    # check which component needs adjustment
                    pass
    ...
    # Specific checks for failed / zero mark records:
    # If a theory subject has ext and int, tot = ext + int
    for i, s in enumerate(SUBJECTS):
        pfx = s["prefix"]
        stype = s["type"]
        cd = comp_data[pfx]
        if stype == "THEORY":
            if cd["ext"] is not None and cd["int"] is not None:
                subj_totals[i] = cd["ext"] + cd["int"]
        elif stype == "TW_ORAL":
            if cd["tw"] is not None and cd["oral"] is not None:
                subj_totals[i] = cd["tw"] + cd["oral"]
        elif stype == "TW_ONLY":
            if cd["tw"] is not None:
                subj_totals[i] = cd["tw"]

    # If any single subject total was missing, resolve via overall total
...
for p in range(2, 47):
    ...
    for sb in seat_boxes:
        rec = build_student_record(p, sb, boxes)
        records.append(rec)
...
print(f"Master CSV successfully saved to: {os.path.abspath(out_csv)}")
print(f"Columns in CSV ({len(master_df.columns)}): {list(master_df.columns)}")
